...
import { AccountManagementSection, type ManagedAccount } from '@/components/dashboard/AccountManagementSection';
...
describe('AccountManagementSection', () => {
  ...
  it('should show reconnect button for expired accounts', () => {
    ...
    const reconnectButton = screen.getByRole('button', { name: /Reconnect Account/ });
  ...
  it('should call onRemoveAccount when deletion confirmed', async () => {
    ...
    // Confirm deletion
    await waitFor(() => {
      const confirmButton = screen.getByRole('button', { name: /Remove Account/ });
  ...
  it('should render empty state when no accounts', () => {
    ...
    expect(screen.getByRole('button', { name: /Connect Your First Account/ })).toBeInTheDocument();
  });
