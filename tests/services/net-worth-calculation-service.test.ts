...
describe("NetWorthCalculationService", () => {
	...
	describe("calculateNetWorth", () => {
		...
		it("should categorize assets correctly", async () => {
			...
			expect(result.asset_breakdown.investments.percentage).toBeCloseTo(
				62.5,
				1,
			); // 25000/40000 * 100
		});
		...
		it("should handle database errors", async () => {
			...
			await expect(service.calculateNetWorth("user-1")).rejects.toThrow(
				"Failed to fetch accounts: Database error",
			);
		});
		...
		it("should handle multiple currencies", async () => {
			...
			expect(mockCurrencyService.normalizeToGBP).toHaveBeenCalledWith(
				1000,
				"USD",
			);
			expect(mockCurrencyService.normalizeToGBP).toHaveBeenCalledWith(
	...
	describe("calculateNetWorthForDate", () => {
		it("should calculate net worth for historical date", async () => {
			...
			const result = await service.calculateNetWorthForDate(
				"user-1",
				"2024-01-01",
			);
