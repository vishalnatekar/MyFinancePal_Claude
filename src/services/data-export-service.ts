...
export class DataExportService {
  ...
  private async gatherExportData(options: ExportOptions) {
    ...
    const history = await this.fetchNetWorthHistory(dateFrom, dateTo, accountIds);

    // Format account data
    const formattedAccounts: AccountExportData[] = (accounts || []).map(acc => ({
      id: acc.id,
  ...
  private generateCSV(data: any): ExportResult {
    ...
    lines.push('Account Name,Type,Institution,Current Balance,Currency,Last Synced');
