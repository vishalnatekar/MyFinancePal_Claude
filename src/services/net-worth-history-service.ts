...
export class NetWorthHistoryService {
  ...
  async getNetWorthTrend(userId: string, dateRange: DateRange): Promise<NetWorthHistoryPoint[]> {
    ...
    const uniqueDates = [...new Set(balanceDates.map(d => d.recorded_at.split('T')[0]))];
  ...
  private async calculateNetWorthForDate(userId: string, date: string): Promise<{
    total: number;
    assets: number;
    liabilities: number;
  }> {
   ...
   */
  async getTrendAnalysis(userId: string, dateRange: DateRange): Promise<{
    ...
    const percentageChange = firstValue !== 0 ? ((lastValue - firstValue) / Math.abs(firstValue)) * 100 : 0;

    // Calculate annualized growth rate
    const daysDiff = Math.max(1, this.daysBetweenDates(history[0].date, history[history.length - 1].date));
    const periodsPerYear = 365 / daysDiff;
    const growthRate = firstValue !== 0 ? (Math.pow(lastValue / Math.abs(firstValue), 1 / periodsPerYear) - 1) * 100 : 0;
