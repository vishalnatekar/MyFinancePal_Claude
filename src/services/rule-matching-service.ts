 ...
 */
export function ruleMatches(rule: SplittingRule, transaction: Transaction): boolean {
 ...
 */
function matchMerchantRule(rule: SplittingRule, transaction: Transaction): boolean {
  ...
  try {
    ...
    console.error(`Invalid regex pattern in rule ${rule.id}: ${rule.merchant_pattern}`, error);
    return false;
  }
}
 ...
 */
function matchCategoryRule(rule: SplittingRule, transaction: Transaction): boolean {
  if (!rule.category_match || !transaction.category) {
    return false;
  }

  // Exact match (case-insensitive)
  return rule.category_match.toLowerCase() === transaction.category.toLowerCase();
}

/**
 * Match amount threshold rule with min/max range
 */
function matchAmountThresholdRule(rule: SplittingRule, transaction: Transaction): boolean {
...
export function logRuleMatch(
  ...
  if (rule) {
    console.log(`[Rule Match] Transaction ${transaction.id} matched rule "${rule.rule_name}" (priority ${rule.priority})`);
  } else {
    console.log(`[Rule Match] Transaction ${transaction.id} did not match any rules`);
  }
}
...
export function validateRuleConfiguration(rule: SplittingRule): {
  ...
  switch (rule.rule_type) {
    ...
    case 'amount_threshold':
      ...
      if (rule.min_amount && rule.max_amount && rule.max_amount <= rule.min_amount) {
  ...
  if (!rule.split_percentage || Object.keys(rule.split_percentage).length === 0) {
    errors.push('split_percentage is required and must have at least one member');
  } else {
    const total = Object.values(rule.split_percentage).reduce((sum, val) => sum + val, 0);
    if (total !== 100) {
