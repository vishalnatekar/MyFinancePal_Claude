...
export class TransactionBasedHistoryService {
	...
	async generateHistoricalTrend(
		...
		console.log(
			`[Transaction-Based History] Found ${transactions?.length || 0} transactions for ${accounts.length} accounts`,
		);
		console.log(
			`[Transaction-Based History] Date range: ${startDate.toISOString().split("T")[0]} to ${endDate.toISOString().split("T")[0]}`,
		);
		...
		return dataPoints.filter(
			(point) => point.net_worth !== 0 || point.assets !== 0,
		);
	}
	 ...
	 */
	private async getCurrentNetWorthOnly(
		userId: string,
	): Promise<NetWorthHistoryPoint[]> {
