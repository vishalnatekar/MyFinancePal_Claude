...
export class NetWorthCalculationService {
	...
	private async getProcessedAccounts(
		userId: string,
	...
	private categorizeAssets(accounts: FinancialAccount[]): AssetBreakdown {
		...
		const totalAssets =
			cash.amount + investments.amount + property.amount + other.amount;
		...
		return {
			...
			investments: {
				...investments,
				percentage:
					totalAssets > 0 ? (investments.amount / totalAssets) * 100 : 0,
			},
			property: {
				...property,
	...
	private calculateCashAssets(accounts: FinancialAccount[]): AssetCategory {
		const cashAccounts = accounts.filter(
			(account) =>
				["checking", "savings"].includes(account.account_type) &&
				account.current_balance > 0,
		);

		return {
			amount: cashAccounts.reduce(
	...
	private calculateInvestmentAssets(
		...
		return {
			amount: investmentAccounts.reduce(
				(sum, account) => sum + account.current_balance,
				0,
			),
			percentage: 0, // Will be calculated in categorizeAssets
	...
	private calculateOtherAssets(accounts: FinancialAccount[]): AssetCategory {
		...
		const otherAccounts = accounts.filter(
			(account) =>
				!["checking", "savings", "investment", "credit", "loan"].includes(
					account.account_type,
				) && account.current_balance > 0,
		);

		return {
			amount: otherAccounts.reduce(
				(sum, account) => sum + account.current_balance,
				0,
			),
			percentage: 0, // Will be calculated in categorizeAssets
	 ...
	 */
	async calculateNetWorthForDate(
		userId: string,
		date: string,
	): Promise<number> {
      ...
      `)
		...
		const accountBalances = new Map<
			string,
			{ balance: number; accountType: string; currency: string }
		>();
