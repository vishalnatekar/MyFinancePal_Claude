...
export async function GET(request: NextRequest) {
  try {
    ...
    const history = await historyService.getNetWorthTrend(user.id, range as DateRange);

    const response: any = { history };

    // Include trend analysis if requested
    if (includeTrend) {
      const trendAnalysis = await historyService.getTrendAnalysis(user.id, range as DateRange);
