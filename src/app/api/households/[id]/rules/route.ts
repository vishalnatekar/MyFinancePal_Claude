...
export async function POST(
  ...
  try {
    ...
    const invalidUserIds = splitUserIds.filter(id => !validUserIds.includes(id));
    ...
    return NextResponse.json({
      ...
      warnings: conflicts.length > 0 ? {
        conflicts: conflicts,
        message: 'This rule may conflict with existing rules based on priority'
      } : undefined
    }, { status: 201 });
...
async function checkRuleConflicts(
  ...
  const conflicts: Array<{ rule_id: string; rule_name: string; reason: string }> = [];

  // Get existing active rules of the same type
  const { data: existingRules } = await supabase
    .from('expense_splitting_rules')
    .select('id, rule_name, rule_type, merchant_pattern, category_match, min_amount, max_amount, priority')
    .eq('household_id', householdId)
    .eq('is_active', true)
  ...
  for (const existing of existingRules) {
    ...
    if (newRule.rule_type === 'merchant' && existing.merchant_pattern && newRule.merchant_pattern) {
    ...
    if (newRule.rule_type === 'category' && existing.category_match === newRule.category_match) {
      conflicts.push({
        rule_id: existing.id,
        rule_name: existing.rule_name,
