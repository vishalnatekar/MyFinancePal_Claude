...
export function AccountSyncStatus({ accounts, onRefresh, loading = false }: AccountSyncStatusProps) {
  ...
  const getStatusBadge = (status: AccountSync['status']) => {
    const variants: Record<AccountSync['status'], 'default' | 'secondary' | 'destructive' | 'outline'> = {
      active: 'default',
      syncing: 'secondary',
      error: 'destructive',
      pending: 'outline',
            ...
            {accounts.map((account) => (
              ...
              >
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <h4 className="font-medium text-sm">{account.accountName}</h4>
                      {getStatusBadge(account.status)}
                    </div>
                    <div className="flex flex-col sm:flex-row sm:gap-4 text-xs text-muted-foreground mt-1">
                      <span>Last synced: {formatLastSynced(account.lastSynced)}</span>
                      {account.status === 'active' && account.nextSync && (
                        <span>Next sync: {formatNextSync(account.nextSync)}</span>
                      )}
                    </div>
                    {account.error && (
                ...
                <Button
                  ...
                  disabled={refreshing[account.accountId] || account.status === 'syncing'}
                >
