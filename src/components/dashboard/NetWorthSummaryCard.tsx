...
export function NetWorthSummaryCard({ netWorth, loading, error, onRetry }: NetWorthSummaryCardProps) {
  if (loading) return <NetWorthSkeleton />;
  if (error) return <ErrorCard message={error} onRetry={onRetry} />;
  if (!netWorth) return <ErrorCard message="No net worth data available" onRetry={onRetry} />;
    ...
    <Card className="net-worth-summary">
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          {netWorth.total_net_worth === 0 && <Minus className="h-5 w-5 text-gray-500" />}
        </CardTitle>
        <CardDescription>
