...
export function SplittingRulesList({
	...
	const fetchRules = async () => {
		try {
			...
			const response = await fetch(
				`/api/households/${householdId}/rules?order_by=priority`,
			);
	...
	const handleToggleActive = async (ruleId: string, currentStatus: boolean) => {
		try {
			const response = await fetch(
				`/api/households/${householdId}/rules/${ruleId}`,
				{
					method: "PUT",
	...
	const getRuleTypeBadge = (ruleType: string) => {
		...
		return (
			<Badge
				className={colors[ruleType as keyof typeof colors] || colors.default}
			>
				{ruleType.replace("_", " ")}
			</Badge>
		);
									...
									<div className="flex items-start gap-4">
										...
										<div>
											...
											<CardDescription className="mt-1">
												Priority {rule.priority} • Created by{" "}
												{rule.creator_name || "Unknown"}
											</CardDescription>
										</div>
									</div>
