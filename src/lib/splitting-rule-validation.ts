 ...
 * Rule type enum
 */
export const RuleTypeSchema = z.enum(['merchant', 'category', 'amount_threshold', 'default']);
 ...
 */
export const SplitPercentageSchema = z.record(z.string().uuid(), z.number().int().min(0).max(100))
  .refine(
    (split) => {
  ...
  .refine(
    (split) => Object.values(split).some(val => val > 0),
    {
      message: 'At least one household member must have a percentage greater than 0',
    }
  );
 ...
 */
export const CreateSplittingRuleSchema = z.object({
 ...
 */
export const UpdateSplittingRuleSchema = z.object({
 ...
 */
export type CreateSplittingRuleRequest = z.infer<typeof CreateSplittingRuleSchema>;
export type UpdateSplittingRuleRequest = z.infer<typeof UpdateSplittingRuleSchema>;
...
export type CreateRuleFromTemplateRequest = z.infer<typeof CreateRuleFromTemplateSchema>;
