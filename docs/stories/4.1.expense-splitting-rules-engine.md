...
- [ ] Task 1: Extend expense_splitting_rules table schema (AC: 2, 3, 4, 5)
  - [ ] Add priority INTEGER field to expense_splitting_rules table (1 = highest priority)
  - [ ] Add rule_type ENUM field ('merchant', 'category', 'amount_threshold', 'default')
  - [ ] Add min_amount and max_amount fields for threshold rules
  - [ ] Add apply_to_existing_transactions BOOLEAN field for bulk application
  - [ ] Create migration file: database/migrations/014_extend_splitting_rules.sql
  - [ ] Add indexes on household_id and priority for efficient querying

- [ ] Task 2: Implement splitting rules CRUD API (AC: 1, 2, 3, 4, 9)
  - [ ] Create POST /api/households/[id]/rules endpoint for creating rules
  - [ ] Create GET /api/households/[id]/rules endpoint for listing rules with priority order
  - [ ] Create PUT /api/households/[id]/rules/[ruleId] endpoint for updating rules
  - [ ] Create DELETE /api/households/[id]/rules/[ruleId] endpoint for deactivating rules
  - [ ] Add Zod validation for rule_type, split_percentage (must sum to 100%), priority
  - [ ] Implement rule conflict detection (warn when rules overlap)

- [ ] Task 3: Build rule priority and matching engine (AC: 5)
  - [ ] Create rule-matching-service.ts in services layer
  - [ ] Implement findMatchingRules(transaction) method returning ordered list by priority
  - [ ] Apply highest priority rule when multiple match
  - [ ] Log rule matches for debugging and testing
  - [ ] Handle merchant_pattern with regex matching (e.g., "Tesco.*")
  - [ ] Handle category_match with exact category matching
  - [ ] Handle amount_threshold with min/max range matching

- [ ] Task 4: Implement bulk rule application to historical transactions (AC: 6)
  - [ ] Create POST /api/households/[id]/rules/[ruleId]/apply endpoint
  - [ ] Query all eligible historical transactions for household
  - [ ] Apply rule to matching transactions (update splitting_rule_id, shared_with_household_id)
  - [ ] Return count of affected transactions
  - [ ] Show progress indicator during bulk application
  - [ ] Allow filtering by date range for targeted application

- [ ] Task 5: Create rule templates system (AC: 7)
  - [ ] Define common rule templates in rule-templates.ts
  - [ ] Templates: "Split groceries 50/50", "Split utilities by custom ratio", "Share all transactions over £100"
  - [ ] Create GET /api/households/[id]/rules/templates endpoint
  - [ ] Create POST /api/households/[id]/rules/from-template endpoint
  - [ ] Allow users to customize template before saving
  - [ ] Show template description and example transactions

- [ ] Task 6: Build rule testing interface (AC: 8)
  - [ ] Create POST /api/households/[id]/rules/test endpoint
  - [ ] Accept draft rule and return matching transactions from last 30 days
  - [ ] Show match count and total amount that would be affected
  - [ ] Display example transactions that match the rule
  - [ ] Allow users to refine rule based on results
  - [ ] Implement dry-run mode (no database changes)

- [ ] Task 7: Build rule management UI components (AC: 1, 2, 3, 4, 5, 9)
  - [ ] Create SplittingRulesList component showing all household rules
  - [ ] Build CreateRuleForm component with rule type selector
  - [ ] Create MerchantRuleForm component for merchant-pattern rules
  - [ ] Create CategoryRuleForm component for category-match rules
  - [ ] Create AmountThresholdRuleForm component for threshold rules
  - [ ] Add SplitPercentageSelector component with member selection and percentages
  - [ ] Implement drag-and-drop priority reordering
  - [ ] Add toggle for active/inactive rules
  - [ ] Show which member created each rule

- [ ] Task 8: Build rule testing and template UI (AC: 7, 8)
  - [ ] Create RuleTestingPanel component
  - [ ] Show preview of matching transactions before saving rule
  - [ ] Add RuleTemplateGallery component with common templates
  - [ ] Implement template customization modal
  - [ ] Show bulk application confirmation dialog with affected count
  - [ ] Add success feedback after bulk application

- [ ] Task 9: Add comprehensive testing coverage
  - [ ] Write unit tests for rule matching engine
  - [ ] Test rule priority resolution with multiple matching rules
  - [ ] Test bulk application logic with various transaction sets
  - [ ] Create component tests for all rule forms
  - [ ] Write E2E test for complete rule creation and application workflow
  - [ ] Test edge cases: no matching transactions, 100% to one person, regex patterns
