...
- [ ] Task 1: Extend transactions table for sharing metadata (AC: 1, 3, 6, 9)
  - [ ] Add `shared_with_household_id` UUID field to transactions table (nullable, references households)
  - [ ] Verify `is_shared_expense` BOOLEAN field exists (already in schema)
  - [ ] Add `shared_at` TIMESTAMP field to track when transaction was shared
  - [ ] Add `shared_by` UUID field (references users) to track who initiated sharing
  - [ ] Create transaction_sharing_history table for audit log
  - [ ] Create migration file: database/migrations/008_transaction_sharing_controls.sql

- [ ] Task 2: Update RLS policies for transaction-level privacy (AC: 3, 4)
  - [ ] Update "Users can view shared transactions" policy to check shared_with_household_id
  - [ ] Ensure private transactions (is_shared_expense = false) are only visible to owner
  - [ ] Add policy preventing users from viewing transactions from households they don't belong to
  - [ ] Test RLS policies with different household membership scenarios

- [ ] Task 3: Implement backend API endpoints for transaction sharing (AC: 1, 5, 8)
  - [ ] Create PUT /api/transactions/[id]/sharing endpoint for single transaction sharing
  - [ ] Create POST /api/transactions/bulk-sharing endpoint for bulk operations
  - [ ] Create GET /api/transactions/[id]/sharing-history endpoint for audit log
  - [ ] Add Zod validation for household_id and sharing status
  - [ ] Implement sharing history tracking in all update operations
  - [ ] Add validation: user must own transaction and belong to target household

- [ ] Task 4: Build transaction sharing control UI components (AC: 1, 2, 5, 7)
  - [ ] Create TransactionSharingToggle component (share/private toggle)
  - [ ] Add sharing status badge to TransactionRow component
  - [ ] Build BulkTransactionSharingControls component for multi-select operations
  - [ ] Create SharingStatusFilter component (all/shared/private dropdown)
  - [ ] Add visual differentiation: green badge for shared, gray badge for private

- [ ] Task 5: Integrate sharing controls into transaction pages (AC: 1, 2, 6, 7)
  - [ ] Update src/app/(dashboard)/transactions/page.tsx with sharing controls
  - [ ] Add sharing toggle column to transaction table
  - [ ] Implement bulk selection toolbar for multi-transaction sharing
  - [ ] Add filter dropdown to show all/shared only/private only
  - [ ] Display household name when transaction is shared (e.g., "Shared with Family")

- [ ] Task 6: Implement household transaction view (AC: 3, 9)
  - [ ] Update GET /api/dashboard/household/[id]/transactions endpoint
  - [ ] Filter to show only transactions where shared_with_household_id matches household
  - [ ] Include owner information (user name) for each shared transaction
  - [ ] Add real-time subscription for new shared transactions via Supabase
  - [ ] Create SharedTransactionsList component with owner labels

- [ ] Task 7: Create transaction sharing audit log interface (AC: 8)
  - [ ] Build TransactionSharingHistory component displaying audit trail
  - [ ] Show who shared transaction, when, and with which household
  - [ ] Add filtering by date range and household
  - [ ] Integrate audit log into transaction details view
  - [ ] Add export functionality for sharing history (CSV format)

- [ ] Task 8: Add comprehensive testing coverage
  - [ ] Write unit tests for transaction sharing API endpoints
  - [ ] Create component tests for TransactionSharingToggle, BulkTransactionSharingControls
  - [ ] Add integration tests for RLS policies with transaction sharing
  - [ ] Write E2E test for sharing workflow (mark transaction shared, verify household sees it)
  - [ ] Test edge cases: sharing to wrong household, revoking sharing, bulk operations
